...
@zedub.zed_cmd(pattern="اكينوتر(?:\s|$)([\s\S]*)")
async def rozdo(e):
    ...
    try:
        ...
        return await e.send_file(
            e.chat_id, aki_photo, caption="**⎉╎حـاول مجـدداً مـرة اخـرى ؟!**"
        )
    if e.out:
        await e.delete()


@zedub.tgbot.on(CallbackQuery(data=re.compile(b"aki_?(.*)")))
@check_owner
async def daj(e):
    ...
    ch = int(dt[0])
    mid = int(dt[1])
    await e.edit("**⎉╎جـارِ التحقق الان انتظـر ...**")
    try:
...
@zedub.tgbot.on(CallbackQuery(data=re.compile(b"aka_?(.*)")))
@check_owner
async def rooks(e):
    ...
    if (
        ...
        ans = [
            await e.builder.photo(
                aki_photo,
                text=query,
                buttons=[Button.inline("⎉ ابـدا اللعب ⎉", data=e.text)],
            )
        ]
        await e.answer(ans)
