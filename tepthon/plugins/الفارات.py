...
ZelzalVP_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝗘𝗣𝗧𝗛𝗢𝗡  𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر الفـارات](t.me/Tepthon) 𓆪\n\n"
    ...
    "⪼ `.اضف كليشة البوت` بالـرد ع الكليشـة لـ إضـافة كليشـة ستـارت\n\n"
    ...
    "⪼ `.وقت المغرب` \n\n"
    "⪼ `.وقت تركيا` \n\n"
    "🛃 سيتـم إضـافة المزيـد من الدول قريبًا\n\n"
    "\n𓆩 [𝙑𝙖𝙧𝙨 - قنـاة الفـارات](t.me/tepthone1) 𓆪"
)
...
async def variable(event):
    ...
    vinfo = reply.text
    zed = await edit_or_reply(event, "** ❈╎جـاري إضـافة الفـار إلــى بـوتك ...**")
    # All Rights Reserved for "محمد - Tepthon"
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        if variable in heroku_var:
            await zed.edit("** ❈╎تم تغييـر {} بنجـاح ☑️**\n** ❈╎المتغيـر : ↶**\n `{}` \n** ❈╎يتم الان اعـادة تشغيـل بـوت تيبثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        else:
            await zed.edit("** ❈╎تم اضافـة {} بنجـاح ☑️** \n** ❈╎المضاف اليه :**\n `{}` \n** ❈╎يتم الان اعـادة تشغيـل بـوت تيبثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await zed.edit("** ❈╎تم حـذف {} بنجـاح ☑️**\n** ❈╎المتغيـر المحـذوف : ↶**\n `{}` \n** ❈╎يتم الان اعـادة تشغيـل بـوت تيبثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, variable))
        ...
        if variable not in heroku_var:
        	return await zed.edit("** ❈╎عـذࢪاً عـزيـزي .. انت لـم تقـم باضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("** ❈╎تم حـذف {} بنجـاح ☑️**\n** ❈╎المتغيـر المحـذوف : ↶**\n `{}` \n** ❈╎يتم الان اعـادة تشغيـل بـوت تيبثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, heroku_var[variable]))
...
async def _(malatha):
    if malatha.fwd_from:
        return
    zed = await edit_or_reply(malatha, "** ❈╎جـاري إضـافة فـار الصـورة إلــى بـوتك ...**")
    ...
    if malatha.reply_to_msg_id:
        ...
        await zed.edit(
            "** ❈╎بالـرد على صـورة لتعييـن الفـار ...**",
        )
